		...
		<view class="notifacation k-active" @click="goDetail(notice.issue.iId)" v-for="(notice, index) in notifacationList" :key="index">
			...
			<view class="notifacation-info">
				<view class="info-main">
					...
					想要参与你的
					<text class="info-special">{{notice.issue.iTitle}}</text>
				</view>
			</view>
			<view class="notifacation-opration">
				<view class="opration-icon allow" @click.native.stop="allow(notice)">
					<text class="iconfont icon-gouxuan"></text>
				</view>
				<view class="opration-icon deny" @click.native.stop="deny(notice)">
		...
		methods: {
			...
			allow(notice) {
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							this.request('issueAttend/adopt', 'POST', {
								iaId: notice.toCommentId
							}).then(res => {
								console.log(res);
								if (res.data.status == 'succ') {
									this.yx.addTeamMembers({
