<template>
    ...
    :default-expanded-keys="[1,2,3]" >
     ...
     <el-dropdown class="sort" >
     <span>
     {{TodayTaskType}} <i class="el-icon-arrow-down el-icon--right"></i>
     </span>
     <el-dropdown-menu slot="dropdown">
     <el-dropdown-item @click.native="chgTodayTaskType('我的创建')">我的创建</el-dropdown-item>
     <el-dropdown-item @click.native="chgTodayTaskType('我的合作')">我的合作</el-dropdown-item>
     </el-dropdown-menu>
     </el-dropdown>
     </div>
...
export default {
    created(){
      let that=this
      console.log(that.$store.state.dbName)
      db=new database({
    ...
    methods:{
        ...
        peekDayPro(type){
          ...
          if(type=='我的创建'){
            ...
            this.cowork.forEach(function(item,i){
              item.plan.process.forEach(function(item_,index){
                 if(index!=0&&index!=item.plan.process.length-1){
                   let proDate_start=that.getDateArr(item_.startDate)
                   let proDate_end=that.getDateArr(item_.endDate)
                   if(proDate_start[0]<=year&&proDate_end[0]>=year&&
                   proDate_start[1]<=month&&proDate_end[1]>=month&&
        ...
        remindMemProgress(mem){
          ...
          this.$http.post(this.$store.state.contextUrl+'/getUserId',this.$qs.stringify({
           ...
           //websocket: send the refresh message
            ...
            //改变今日任务类型
            chgTodayTaskType(type){
              this.TodayTaskType=type
            }
    },
    watch:{
      'goalAddVisible':{
