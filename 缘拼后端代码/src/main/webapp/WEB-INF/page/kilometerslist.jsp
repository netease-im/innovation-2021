...
layui.config({
	...
	table.render({
		...
		,url: 'kilometers/listjson' //修改这个地址，列表查询路径，数据类型见json/table/demo.js
		,toolbar: '#test-table-toolbar-toolbarDemo'
		,title: '用户数据表'
		,cols: [[
			 {filed:'id',title:'方案', type:'numbers', width:100, fixed: 'left' }
			,{field:'kilometers', title:'公里数', fixed: 'left'}
			,{field:'remarks', title:'备注', fixed: 'left'}
    ...
    table.on('toolbar(test-table-toolbar)', function(obj){
		if(obj.event=="add"){
			parent.layui.index.openTabsPage("user/add", "添加方案");
		}
    });
	...
	table.on('edit(test-table-toolbar)', function(obj){ 
		...
		admin.req({
			type: "POST"
			,url: 'kilometers/echodata' //实际使用请改成服务端真实接口
			,data: data
			,done: function(res){
    ...
    table.on('tool(test-table-toolbar)', function(obj){
		...
		var user=obj.data;
		if(obj.event === 'edit'){
			parent.layui.index.openTabsPage("kilometers/echodata?id="+obj.data.id, "修改方案");
		}
    }); 
});
